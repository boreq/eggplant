<template>
    <ul class="tracks">
        <li v-for="(entry, index) in entries" :key="entry.track.id" class="track" :class="{ playing: isNowPlaying(index, entry.track), highlight: isHighlighted(entry.track) }" @click="onTrackRowClicked(index)">
            <div class="play">
